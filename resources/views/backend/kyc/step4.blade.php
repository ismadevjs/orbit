        ...
        <div class="col-lg-8">
            <div class="card shadow-sm border-0">
                <div class="card-header bg-primary ">
                    <h4 class="mb-0">التحقق من الهوية - الخطوة الرابعة</h4>
                    <small>التقاط صورة الإقامة</small>
                </div>

                <div class="card-body">
                    <form id="kycForm" method="POST" action="{{ route('investor.kyc.step.four.submit') }}" dir="rtl">
                        @csrf
                        ...
                        <!-- Image Preview -->
                        <div class="mb-3">
                            <h6 class="form-label fw-bold">معاينة صورة الإقامة</h6>
                            <div id="residencyPreview">
                                @if ($kyc && $kyc->residency_photo_path)
                                    <img src="{{ Storage::url($kyc->residency_photo_path) }}" class="preview-image" alt="صورة الإقامة">
                                @endif
                            </div>
                        </div>

                        <!-- Hidden Input for Base64 Image -->
                        <input type="hidden" name="residency_image" id="residency_image">
...
@push('scripts')
    <script>
        ...
        const residencyPreview = document.getElementById('residencyPreview');
        const residencyImageInput = document.getElementById('residency_image');
        const submitBtn = document.getElementById('submitBtn');
        const errorMessage = document.getElementById('errorMessage');

        let stream;
        let facingMode = 'environment'; // Default to rear camera for document
        ...
        captureBtn.addEventListener('click', (e) => {
            ...
            residencyPreview.innerHTML = '';
            residencyPreview.appendChild(img);
            residencyImageInput.value = imageData;

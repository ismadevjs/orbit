    ...
    @section('content')
        <div class="container">
            <div class="card shadow-lg border-0">
                ...
                <form action="{{ route('stuff.permissions.update') }}" method="POST" class="p-4">
                    @csrf
                    ...
                    <div class="mb-5 row align-items-center">
                        ...
                        <div class="col-md-9">
                            <input type="text" id="name" name="name" value="{{ $role->name }}"
                                   ...
                                   placeholder="اسم الدور" required>
                        </div>
                    </div>
                                        ...
                                        @foreach (permissions_list() as $pl)
                                            <div class="col-lg-3 col-md-4 col-sm-6">
                                                <div class="permission-item text-center p-3 rounded border bg-white shadow-sm">
                                                    <label class="form-check-label d-block fw-semibold mb-2"
                                                           for="{{ $pl }}-{{ $r }}">
                                                    ...
                                                    <div class="form-check form-switch d-flex justify-content-center">
                                                        <input type="checkbox" name="permissions[]"
                                                               ...
                                                               id="{{ $pl }}-{{ $r }}"
                                                               value="{{ $pl }}-{{ $r }}"
                                                               @foreach ($role->permissions as $rls)
                                                                   @if($rls->name == $pl . ' ' . $r) checked @endif
                                                            @endforeach>
                                                    </div>
                                                </div>
                                            </div>
